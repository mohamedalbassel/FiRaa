...
namespace Egyptian_FLS_code
{
    ...
    public class CMDFLS : IExternalCommand
    {
        XYZ q = new XYZ();
        ...
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            try
            {
                ...
                //if ((WindowsIdentity.GetCurrent().Name.Substring(0, 5).ToLower() != @"dmsd\") ^ (WindowsIdentity.GetCurrent().Name.Substring(0, 9).ToLower() != @"daramman\") ^ (WindowsIdentity.GetCurrent().Name.Substring(0, 10).ToLower() != @"darbeirut\") ^ (WindowsIdentity.GetCurrent().Name.Substring(0, 4).ToLower() != @"dar"))
                //{

                //    throw new Exception(String.Format("This application only for dar cairo users \n if you need help call Mohamed Albassel at ext:1027 DAR Cairo  \n mohamed.albassel@dar.com   "));
                //}
                    ...
                    if (Form1.travel == true)
                    {
                        using (Transaction txr = new Transaction(doc, "new Travel Path"))
                        {
                            ...
                            try
                            {
                                ...
                                //System.Windows.MessageBox.Show(rms.Count().ToString());
                            ...
                            //routeMap = PathOfTravel.CreateMapped(uiDocument.ActiveView, ptclrr, p);
